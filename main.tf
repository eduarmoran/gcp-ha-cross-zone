...
resource "google_compute_network" "vpc_network" {
  name                    = "public-1-vpc-${random_string.random_name_post.result}"
  auto_create_subnetworks = false
}

resource "google_compute_network" "vpc_network4" {
  name                    = "public-2-vpc-${random_string.random_name_post.result}"
  auto_create_subnetworks = false
}

resource "google_compute_network" "vpc_network2" {
  name                    = "private-vpc-${random_string.random_name_post.result}"
  auto_create_subnetworks = false
}

resource "google_compute_network" "vpc_network3" {
  name                    = "syncmgmt-vpc-${random_string.random_name_post.result}"
  auto_create_subnetworks = false
}

### Public Subnet 1 ###
resource "google_compute_subnetwork" "public_subnet" {
  name                     = "public-subnet-1-${random_string.random_name_post.result}"
  region                   = var.region
...
### Public Subnet 2 ###
resource "google_compute_subnetwork" "public2_subnet" {
  name                     = "public-subnet-2-${random_string.random_name_post.result}"
  region                   = var.region
  network                  = google_compute_network.vpc_network4.name
  ip_cidr_range            = var.public2_subnet
  private_ip_google_access = true
}
...
resource "google_compute_subnetwork" "ha_subnet" {
  name          = "syncmgmt-subnet-${random_string.random_name_post.result}"
  region        = var.region
...
resource "google_compute_firewall" "allow-fgt" {
  name    = "allow-public-1-${random_string.random_name_post.result}"
  network = google_compute_network.vpc_network.name
...
resource "google_compute_firewall" "allow-sync" {
  name    = "allow-syncmgmt-${random_string.random_name_post.result}"
  network = google_compute_network.vpc_network3.name
...
# Create static cluster ip 1
resource "google_compute_address" "static" {
  name = "cluster-ip1-${random_string.random_name_post.result}"
}

# Create static cluster ip 2
resource "google_compute_address" "static8" {
  name = "cluster-ip2-${random_string.random_name_post.result}"
}
...
resource "google_compute_instance" "default" {
  name           = "fotigate-01-${random_string.random_name_post.result}"
...
network_interface {
    subnetwork = google_compute_subnetwork.public2_subnet.name
    nic_type   = var.nictype
    network_ip = var.active_port4_ip
    access_config {
      nat_ip = google_compute_address.static8.address
    }
  }
  ...
  metadata = {
    user-data = templatefile("${path.module}/active", {
      ...
      clusterip1           = "cluster-ip1-${random_string.random_name_post.result}"
      clusterip2           = "cluster-ip2-${random_string.random_name_post.result}"
...
resource "google_compute_instance" "default2" {
  name           = "fotigate-02-${random_string.random_name_post.result}"
  machine_type   = var.machine
  zone           = var.zone2
  can_ip_forward = "true"
